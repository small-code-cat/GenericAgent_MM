...
{
  "image_description": "[类别标签] 简洁的图片内容描述（仅含图片时填写，纯文本输入留空字符串）",
  "knowledge": "[类别标签] 对内容的精简综合理解（提炼核心要点，不超过200字）"
}

要求：
1. image_description 和 knowledge 必须以类别标签作为前缀，格式为 [类别]，例如：
   - [人物照片] [车辆照片] [宠物照片] [风景照片] [文档截图] [聊天记录]
   - [个人信息] [车辆信息] [宠物信息] [工作笔记] [生活记录] 等
   类别应根据内容语义自行判断，不限于以上示例
2. image_description 只描述图片视觉内容，精简扼要，但要包含关键识别特征
3. knowledge 是对图文内容的综合理解和提炼，精简但保留核心信息
4. 只输出 JSON，不要其他内容"""
