...
## macOS 安装流程（已验证）
1. `open -a "Google Chrome"` 打开浏览器
2. TM 新建脚本 → `cat assets/ljq_web_driver.user.js | pbcopy` → 用户 Cmd+A Cmd+V Cmd+S
3. 必须打开普通 http(s) 网页（chrome-extension:// 页不运行 userscript）
4. `web_scan(tabs_only=True)` 验证连通
